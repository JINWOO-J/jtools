...
def arrayToDict(obj, key):
    return_result = {}
    for item in obj:
        return_result[item.get(key)] = item
...
def main():
    ...
    parser.add_argument('--timeout', metavar='timeout', type=int, help=f'timeout', default=5)
    ...
    check_targets, prep_infos = get_ip_list()
    prep_infos_dict = arrayToDict(prep_infos, "address")
    ...
    if args.writeconfig:
        ...
        if args.verbose:
            ...
            for i, item in enumerate(sorted_list):
                grade = "NULL"
                address = item.get('peer_id')
                if prep_infos_dict.get(address):
                    grade = prep_infos_dict.get(address).get('grade')
                print_debug(f"[{i:>2}] [{grade}] {item.get('location')}, IPADDR: {item['peer_ip']:<18}, ResponseTime: {item['elapsed']:>3}ms, Avail: {item['service_available']}, State: {item['state']}, BlockHeight: {item['block_height']}")
